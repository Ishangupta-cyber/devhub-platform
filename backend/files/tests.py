...
class FileNodeAPITests(APITestCase):
    ...
    def test_move_file_to_folder(self):
        ...
        self.as_owner()
        readme = FileNode.objects.create(
            repository=self.repo, name='README.md',
            node_type=FileNode.NodeType.FILE,
        )
        res = self.client.patch(self.detail_url(readme.pk),
                                {'parent': self.src.pk}, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK, res.data)
        self.assertEqual(res.data['path'], 'src/README.md')
    ...
    def test_move_to_root(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK, res.data)
        self.assertEqual(res.data['path'], 'Button.jsx')
        self.assertIsNone(res.data['parent'])

    def test_rename_does_not_move_to_root(self):
        
        self.as_owner()
        res = self.client.patch(self.detail_url(self.button.pk),
                                {'name': 'renamed.js'}, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK, res.data)
        self.assertEqual(res.data['parent'], self.src.pk)
        self.assertEqual(res.data['path'], 'src/renamed.js')

    def test_move_into_own_descendant_rejected(self):
        self.as_owner()
        comp = FileNode.objects.create(
            repository=self.repo, parent=self.src, name='components',
            node_type=FileNode.NodeType.FOLDER,
        )
        utils = FileNode.objects.create(
            repository=self.repo, parent=comp, name='utils',
            node_type=FileNode.NodeType.FOLDER,
        )
        res = self.client.patch(self.detail_url(self.src.pk),
                                {'parent': utils.pk}, format='json')
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST, res.data)
        self.assertIn('descendant', str(res.data))
    ...
    def test_move_to_destination_with_same_name_rejected(self):
        self.as_owner()
        FileNode.objects.create(
            repository=self.repo, name='Button.jsx',
            node_type=FileNode.NodeType.FILE,
        )
        res = self.client.patch(self.detail_url(self.button.pk),
                                {'parent': None}, format='json')
    ...
    def test_move_to_other_repository_rejected(self):
        self.as_owner()
        res = self.client.patch(self.detail_url(self.button.pk),
                                {'parent': self.other_folder.pk}, format='json')
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST, res.data)
        self.assertIn('Parent folder nahi mila', str(res.data))

    def test_move_into_file_rejected(self):
        self.as_owner()
        other = FileNode.objects.create(
            repository=self.repo, name='x.js',
            node_type=FileNode.NodeType.FILE,
        )
        res = self.client.patch(self.detail_url(other.pk),
                                {'parent': self.button.pk}, format='json')
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST, res.data)
        self.assertIn('folder hona chahiye', str(res.data))
