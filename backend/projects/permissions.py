...
class IsRepoOwnerForColumn(BasePermission):
    def has_permission(self, request, view):
        ...
        return project.repository.user_can_manage(request.user)

    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return True
        return obj.project.repository.user_can_manage(request.user)
...
class IsRepoOwnerForCard(BasePermission):
    def has_permission(self, request, view):
        ...
        return column.project.repository.user_can_manage(request.user)

    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return True
        return obj.column.project.repository.user_can_manage(request.user)